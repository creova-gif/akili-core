...
class LinkedInClient:
    ...
    async def post_text(self, account_key: str, text: str) -> dict:
        ...
        if not acc["urn"]:
            return {"error": f"Missing URN for {acc['label']} — add to Replit Secrets"}
    ...
    async def format_status(self) -> str:
        ...
        if "error" not in profile:
            first = profile.get("firstName", {}).get("localized", {}).get("en_US", "")
            last = profile.get("lastName", {}).get("localized", {}).get("en_US", "")
            return f"💼 LINKEDIN STATUS\n  ✅ {first} {last} (personal) + CREOVA page — connected"
