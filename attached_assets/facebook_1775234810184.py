...
class FacebookClient:
    ...
    async def post_text(self, account_key: str, message: str) -> dict:
        ...
        async with aiohttp.ClientSession() as s:
            async with s.post(
                ...
                log.error(f"[Facebook] Post error for {acc['label']}: {data}")
    ...
    async def post_link(self, account_key: str, link: str, message: str) -> dict:
        """Post a link (great for driving traffic to creova.one)."""
        acc = self._get_account(account_key)
        async with aiohttp.ClientSession() as s:
            async with s.post(
                f"{BASE_URL}/{acc['page_id']}/feed",
    ...
    async def post_to_both(self, justin_message: str, creova_message: str) -> list:
        """Post different copy to both Facebook accounts simultaneously."""
        results = []
    ...
    async def get_page_insights(self, account_key: str) -> dict:
        """Get page fan count and engagement."""
        acc = self._get_account(account_key)
        url = (
    ...
    async def format_status(self) -> str:
        ...
        for key in self.accounts:
            try:
                ...
                name = insights.get("name", self.accounts[key]["label"])
                ...
                lines.append(f"  ❌ {self.accounts[key]['label']} — {str(e)[:50]}")
        return "\n".join(lines)
