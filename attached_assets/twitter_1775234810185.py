...
class TwitterClient:
    ...
    async def post_tweet(self, text: str, reply_to: Optional[str] = None) -> dict:
        ...
        try:
            ...
            tweet_id = r.data["id"]
            log.info(f"[Twitter] ✅ Tweeted as @justin_mafie — {tweet_id}")
            return {"success": True, "tweet_id": tweet_id}
        except Exception as e:
            log.error(f"[Twitter] Post error: {e}")
            return {"error": str(e)}

    async def post_thread(self, tweets: list[str]) -> list:
    ...
    def build_tweet(self, message: str, include_tags: bool = True) -> str:
        """Build a branded tweet for @justin_mafie."""
        base = message.strip()
        tags = "\n\n#JustinMafie #CREOVA #AfricanFounder #CREOVAMusic" if include_tags else ""
        full = f"{base}{tags}"
        return self._trim(full)

    def build_thread_from_topic(self, topic: str, points: list[str]) -> list[str]:
        """
        Turn a topic + bullet points into a ready-to-post thread.
        Automatically adds opener and closer tweets.
        """
