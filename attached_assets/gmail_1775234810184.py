...
class GmailClient:
    ...
    def _init_service(self, account_key: str) -> bool:
        ...
        try:
            ...
            log.info(f"Gmail ✅ {acc['label']} connected — {acc['address']}")
            return True

        except ImportError:
            log.warning("Google libraries not installed — pip install google-api-python-client google-auth-oauthlib")
            return False
    ...
    def _get_service(self, account_key: str):
        ...
        if not svc:
            raise ValueError(f"Gmail not initialized for '{account_key}'. Run auth first.")
        return svc

    async def get_unread(self, account_key: str, max_results: int = 20) -> list:
        """Get unread emails from an account."""
        ...
        try:
            ...
            log.info(f"[Gmail] {account_key}: {len(emails)} unread emails")
            return emails
        except Exception as e:
    ...
    def classify_email(self, email: dict) -> str:
        """Classify an email by type for smart routing."""
    ...
    def route_reply(self, email: dict) -> str:
        """
        Decide which email account to reply FROM based on the incoming email.
